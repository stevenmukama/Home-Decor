  ...
  return (
    <div className="relative overflow-hidden bg-white rounded shadow-md">
      <div className="relative overflow-hidden rounded-lg">
        <Link to={`/product/${prodId}`}>
          <img
            src={product.image}
            alt={product.title}
            className="object-cover w-full h-64 hover:opacity-70"
          />
        </Link>
        ...
        >
          <SolidHeartIcon className="w-6 h-6" onClick={toggleFavorite} />
        </button>
      </div>
      <div className="px-4 py-4">
        ...
        <button
          className={`py-1 text-sm font-bold text-white bg-gray-400 rounded-lg px-10 hover:bg-gray-500 no-wrap ${
